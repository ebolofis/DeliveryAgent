...
namespace DeliveryAgent.Helpers
{
    public static class CultureHelper
    {
        ...
        private static readonly List<string> implementedCultures = new List<string> { "el", "en" };
        ...
        public static string GetImplementedCulture(string name)
        {
            ...
            if (implementedCultures.Where(c => c.Equals(name, StringComparison.InvariantCultureIgnoreCase)).Count() > 0)
            {
                return name;
            }
            var neutralCulture = GetNeutralCulture(name);
            foreach (var culture in implementedCultures)
            {
